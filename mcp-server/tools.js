...
function createTools(apiRequest) {
  ...
  {
    name: 'list_restaurant_web_orders',
    description: "\"Nando's style\" food orders placed and paid on the website (not the table-tab flow) -- pickup or at-table, no running tab",
    inputSchema: {
      restaurant_id: z.string().optional(),
      status: z.enum(['pending', 'paid', 'preparing', 'ready', 'completed', 'cancelled']).optional(),
      cursor: z.string().optional().describe('created_at of the last item from the previous page'),
      limit: z.number().int().optional(),
    },
    run: (args) => apiRequest('GET', '/api/restaurant-web-orders', { query: args }),
  },
  {
    name: 'get_restaurant_web_order',
    description: 'Get a website food order with its line items',
    inputSchema: { id: z.string() },
    run: ({ id }) => apiRequest('GET', `/api/restaurant-web-orders/${id}`),
  },
  {
    name: 'create_restaurant_web_order',
    description: 'Create a website food order (pickup, or at-table if table_id is set). Item prices always come from the live menu, never from this call. No payment is taken here; that happens through the website\'s own checkout (payment-intent + confirm-payment), not via MCP.',
    inputSchema: {
      restaurant_id: z.string(),
      table_id: z.string().optional().describe('Set only for an at-table order; omit for pickup'),
      contact_name: z.string().optional(),
      contact_email: z.string().optional(),
      contact_phone: z.string().optional(),
      scheduled_for: z.string().optional().describe('ISO datetime for collection/arrival; omit for ASAP'),
      notes: z.string().optional(),
      items: z.array(z.object({ item_id: z.string(), quantity: z.number().int().min(1), variant: z.string().optional() })),
    },
    run: (args) => apiRequest('POST', '/api/restaurant-web-orders', { body: args }),
  },
  {
    name: 'update_restaurant_web_order_status',
    description: "Update a website food order's kitchen status (paid -> preparing -> ready -> completed, or cancelled)",
    inputSchema: { id: z.string(), status: z.enum(['pending', 'paid', 'preparing', 'ready', 'completed', 'cancelled']) },
    run: ({ id, status }) => apiRequest('PUT', `/api/restaurant-web-orders/${id}`, { body: { status } }),
  },
  {
