...
function createTools(apiRequest) {
  ...
  {
    name: 'list_tours',
    description: 'List active tours (including scheduled services like a ferry) for this property',
    inputSchema: {},
    run: () => apiRequest('GET', '/api/tours'),
  },
  {
    name: 'create_tour',
    description: 'Create a tour. For a fixed-schedule service (e.g. a ferry, one tour per direction), set departure_times and slots are generated automatically ~6 months ahead; otherwise add slots with bulk_create_tour_slots.',
    inputSchema: {
      name: z.string(),
      description: z.string().optional(),
      duration_mins: z.number().int(),
      max_group_size: z.number().int().describe('Seats per departure'),
      price: z.number().describe('Price per person'),
      departure_times: z.array(z.string()).optional().describe('Daily departure times, e.g. ["08:00", "12:30"]'),
      departure_days: z.array(z.number().int().min(0).max(6)).optional().describe('Weekdays it runs, 0 = Sunday .. 6 = Saturday; omit for every day'),
    },
    run: (args) => apiRequest('POST', '/api/tours', { body: args }),
  },
  {
    name: 'update_tour',
    ...
    inputSchema: {
      ...
      max_group_size: z.number().int().optional(),
      price: z.number().optional(),
      status: z.enum(['active', 'inactive']).optional(),
      departure_times: z.array(z.string()).optional(),
      departure_days: z.array(z.number().int().min(0).max(6)).nullable().optional(),
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      times: z.array(z.string()).describe('e.g. ["09:00", "14:00"]'),
    },
    run: (args) => apiRequest('POST', '/api/tours/slots/bulk', { body: args }),
  },
  {
    ...
    run: (args) => apiRequest('GET', '/api/tours/slots/search', { query: args }),
  },
  {
  ...
  },
  {
    ...
    description: 'List tour bookings, optionally filtered by date, status, or guest',
    inputSchema: {
      date: z.string().optional(),
      status: z.string().optional(),
      guest_id: z.string().optional(),
    },
    run: (args) => apiRequest('GET', '/api/tours/bookings', { query: args }),
  },
  {
    name: 'create_tour_booking',
    description: 'Book seats on a tour slot. total_price is computed server-side from the tour price and group size; fails with 409 if the slot lacks seats.',
    inputSchema: {
      slot_id: z.string(),
      guest_id: z.string().optional(),
    ...
    run: (args) => apiRequest('POST', '/api/tours/bookings', { body: args }),
  },
  {
    name: 'update_tour_booking',
    description: 'Update a tour booking (e.g. set status to "cancelled" to cancel it)',
    inputSchema: {
      id: z.string(),
      status: z.string().optional(),
      notes: z.string().optional(),
    },
    run: ({ id, ...body }) => apiRequest('PUT', `/api/tours/bookings/${id}`, { body }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      product_group: z.string().optional().describe('Shared key (also the displayed product name) grouping colour/size variants -- e.g. "Trucker Hat" for items named "Trucker Hat (Black)"/"(Grey)"/"(Pink)". Omit for a standalone item.'),
      variant_label: z.string().optional().describe('This item\'s option value within its product_group, e.g. "Black"'),
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      price: z.number().optional(),
      status: z.enum(['active', 'inactive']).optional(),
      stock_quantity: z.number().int().min(0).nullable().optional().describe('null clears it back to untracked/unlimited'),
      product_group: z.string().nullable().optional().describe('null ungroups the item'),
  ...
  {
    name: 'list_proshop_orders',
    description: 'List standalone pro shop orders (guest checkout from the venue website, not attached to a booking)',
    inputSchema: {
      shop_id: z.string().optional(),
    ...
    run: (args) => apiRequest('GET', '/api/proshop/orders', { query: args }),
  },
  {
    name: 'get_proshop_order',
    description: 'Get a pro shop order with its line items',
    inputSchema: { id: z.string() },
    run: ({ id }) => apiRequest('GET', `/api/proshop/orders/${id}`),
  },
  {
    name: 'create_proshop_order',
    description: 'Create a standalone pro shop order (not attached to a booking). Item prices always come from the live catalogue, never from this call -- pass item_id/quantity pairs only. No payment is taken here; that happens through the website\'s own checkout (payment-intent + confirm-payment), not via MCP.',
    inputSchema: {
      shop_id: z.string(),
      contact_name: z.string(),
      contact_email: z.string().optional(),
      ...
      shipping_cost: z.number().min(0).optional().describe('Computed by the ordering website (carrier rates) -- omit for none/pickup'),
      notes: z.string().optional(),
      items: z.array(z.object({ item_id: z.string(), quantity: z.number().int().min(1) })),
    },
    run: (args) => apiRequest('POST', '/api/proshop/orders', { body: args }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      lead_time_hours: z.number().int().min(1).max(720).optional().describe('Minimum notice in hours for a guest booking. Omit for none'),
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      lead_time_hours: z.number().int().min(1).max(720).nullable().optional().describe('Minimum notice in hours for a guest booking (staff bookings are exempt). null = none'),
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      member_email: z.string().optional().describe("A verified client email; dates its regulars' rate covers use the treatment's member duration"),
    },
    run: ({ spa_id, ...query }) => apiRequest('GET', `/api/spa/${spa_id}/availability`, { query }),
  },
  {
    name: 'get_spa_member_rate',
    description: "Last appointment date (member_until) an email gets the regulars' rate for: a checked-in/completed/finished visit within the previous 28 days, or a staff-granted guest.member_until. null = not a regular.",
    inputSchema: { spa_id: z.string(), email: z.string() },
    run: ({ spa_id, email }) => apiRequest('GET', `/api/spa/${spa_id}/member-rate`, { query: { email } }),
  },
  {
  ...
  },
  {
    ...
    description: "Create a spa treatment. member_price is the regulars' rate (clients whose last visit was within 28 days); price may be omitted for a regulars-only treatment that has a member_price.",
    inputSchema: {
      spa_id: z.string(),
      name: z.string(),
      description: z.string().optional(),
  ...
  },
  {
    ...
    inputSchema: {
      ...
      contact_phone: z.string().optional(),
      notes: z.string().optional(),
      member_email: z.string().optional().describe("Computed flow: an email you've verified belongs to the client; books at the regulars' rate if it qualifies. Never inferred from contact_email."),
