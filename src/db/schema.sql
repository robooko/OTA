...
-- adding retail items to an existing tee time). See
-- migrate-2026-09-05-proshop-orders.sql. Payment mirrors
-- restaurant_table_session's 'online' channel: property.stripe_secret_key,
-- a regular PaymentIntent verified server-side before payment_status flips.
-- shipping_cost is trusted as given (the ordering website computes it,
-- carrier rates are out of scope here); items_subtotal is always computed
-- server-side from proshop_item.price, never trusted from the client.
