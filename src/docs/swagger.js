const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/restaurant-orders/menu': {
      ...
      post: { tags: ['Restaurant Orders'], summary: 'Create menu item', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['name', 'price'], properties: { name: { type: 'string' }, description: { type: 'string' }, category: { type: 'string' }, price: { type: 'number' }, restaurant_id: { type: 'string', format: 'uuid', description: 'Optional — which restaurant this item belongs to' }, allergens: { type: 'array', items: { type: 'string' } }, translations: { type: 'object', additionalProperties: true, description: 'Keyed by language code, e.g. { "fr": { "name": "...", "description": "..." } }', example: { fr: { name: 'Steak frites', description: 'Servi avec une sauce au poivre' } } } } } } } }, responses: { 201: { description: 'Created' } } },
    },
    '/api/restaurant-orders/menu/{id}': {
      put: { tags: ['Restaurant Orders'], summary: 'Update menu item', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { name: { type: 'string' }, description: { type: 'string' }, category: { type: 'string' }, price: { type: 'number' }, status: { type: 'string', enum: ['active', 'inactive'] }, restaurant_id: { type: 'string', format: 'uuid' }, allergens: { type: 'array', items: { type: 'string' } }, translations: { type: 'object', additionalProperties: true, description: 'Keyed by language code -- replaces the whole object, not a per-language merge', example: { fr: { name: 'Steak frites', description: 'Servi avec une sauce au poivre' } } } } } } } }, responses: { 200: { description: 'Updated' }, 404: { description: 'Not found' } } },
    },
