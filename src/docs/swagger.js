const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/proshop/orders': {
      get: { tags: ['Pro Shop'], summary: 'List standalone orders (guest checkout from the website, not attached to a booking)', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'shop_id', in: 'query', schema: { type: 'string', format: 'uuid' } }, { name: 'status', in: 'query', schema: { type: 'string', enum: ['pending', 'paid', 'cancelled'] } }, { name: 'cursor', in: 'query', schema: { type: 'string', format: 'date-time' }, description: "Oldest loaded order's created_at" }, { name: 'limit', in: 'query', schema: { type: 'integer', default: 30, maximum: 100 } }], responses: { 200: { description: 'Array of orders' } } },
      post: { tags: ['Pro Shop'], summary: 'Create an order', description: 'Item prices are always looked up from the live catalogue server-side -- a client-supplied price is never trusted. shipping_cost is trusted as given (the ordering website computes it from its own carrier rates); items_subtotal + shipping_cost = total_price. No payment is taken by this call.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['shop_id', 'contact_name', 'items'], properties: { shop_id: { type: 'string', format: 'uuid' }, contact_name: { type: 'string' }, contact_email: { type: 'string' }, contact_phone: { type: 'string' }, shipping_address: { type: 'string' }, shipping_cost: { type: 'number', default: 0 }, notes: { type: 'string' }, items: { type: 'array', items: { type: 'object', required: ['item_id', 'quantity'], properties: { item_id: { type: 'string', format: 'uuid' }, quantity: { type: 'integer', minimum: 1 } } } } } } } } }, responses: { 201: { description: 'Created order with its line items' }, 400: { description: 'An item is missing/inactive/not in this shop, or a bad quantity/shipping_cost' }, 404: { description: 'Shop not found' }, 409: { description: 'Not enough stock for one of the items' } } },
    },
