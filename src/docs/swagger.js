const swaggerSpec = {
  ...
  paths: {
    ...
    // ── Property ────────────────────────────────────────────────────────────
    '/api/property/me': {
      get: { tags: ['Property'], summary: 'Get the current property (id, name, currency, timezone)', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], responses: { 200: { description: 'Property identity', content: { 'application/json': { schema: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, name: { type: 'string' }, currency: { type: 'string' }, timezone: { type: 'string' } } } } } } } },
