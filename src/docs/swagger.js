const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/property/google-reviews': {
      get: { tags: ['Property'], summary: "Live Google reviews for this property's website", description: 'Rating, count, latest quotes, maps link and write-a-review link for the property\'s google_place_id, fetched from the Places API and cached server-side (12h TTL; stale cache served on Google failure). 404 when no place id is set; 503 when the server has no GOOGLE_PLACES_API_KEY and nothing is cached.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], responses: { 200: { description: '{ rating, count, maps_url, write_review_url, reviews: [{ author, author_photo, rating, text, relative_time, published_at }], fetched_at }' }, 404: { description: 'No Google place id configured for this property' }, 502: { description: 'Google fetch failed and no cache exists' }, 503: { description: 'Feature not configured on this server and no cache exists' } } },
    },
