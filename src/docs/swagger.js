const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/proshop/orders/{id}': {
      ...
      put: { tags: ['Pro Shop'], summary: "Update an order's status and/or contact details", description: "Per field: omit = unchanged. Covers staff use (status only) and a website checkout patching in the real contact details once the guest has entered them -- createOrder accepts no contact_name so a PaymentIntent can exist before that form even renders; this fills it in before confirm-payment.", security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { status: { type: 'string', enum: ['pending', 'paid', 'cancelled'] }, contact_name: { type: 'string' }, contact_email: { type: 'string' }, contact_phone: { type: 'string' }, shipping_address: { type: 'string' }, notes: { type: 'string' } } } } } }, responses: { 200: { description: 'Updated order' }, 400: { description: 'Invalid status' }, 404: { description: 'Not found' } } },
    },
    '/api/proshop/orders/{id}/payment-intent': {
      post: { tags: ['Pro Shop'], summary: 'Create (or reuse) a Stripe PaymentIntent for the order total', description: "Same pattern as restaurant table sessions' 'online' channel: a regular (not card_present) PaymentIntent on the property's own Stripe account (property.stripe_secret_key), embeddable via Stripe Elements. Idempotent -- a retry reuses the existing intent unless it's already succeeded/cancelled.", security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], responses: { 200: { description: '{ client_secret, payment_intent_id, amount } in minor units, or { already_paid: true, payment_intent_id } if a prior charge already succeeded' }, 404: { description: 'Order not found' }, 409: { description: 'No Stripe key configured, order cancelled, or already paid' }, 502: { description: 'Stripe error' } } },
    },
    '/api/proshop/orders/{id}/confirm-payment': {
      post: { tags: ['Pro Shop'], summary: 'Verify a succeeded PaymentIntent and mark the order paid', description: 'Re-checks the PaymentIntent directly with Stripe (status and amount) before trusting a client-side "it succeeded" claim -- the same verification restaurant table sessions and spa Tap to Pay use.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['payment_intent_id'], properties: { payment_intent_id: { type: 'string' } } } } } }, responses: { 200: { description: 'Order, now status/payment_status = paid' }, 404: { description: 'Not found' }, 409: { description: "Intent doesn't match this order, hasn't succeeded, or amount mismatch" }, 502: { description: 'Stripe error' } } },
    },
