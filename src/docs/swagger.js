const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/spa/{id}': {
      ...
      put: { tags: ['Spa'], summary: 'Update spa. Set status to "inactive" to delete it — there is no hard-delete endpoint.', security: [{ bearerAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { content: { 'application/json': { schema: { type: 'object', properties: { name: { type: 'string' }, description: { type: 'string' }, phone: { type: 'string' }, status: { type: 'string', enum: ['active', 'inactive'] }, slot_interval_minutes: { type: 'integer' }, contact_email: { type: 'string' }, address: { type: 'string' } } } } } }, responses: { 200: { description: 'Updated' } } },
    },
