const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/proshop/orders/{id}': {
      ...
      put: { tags: ['Pro Shop'], summary: "Update an order's status", security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['status'], properties: { status: { type: 'string', enum: ['pending', 'paid', 'cancelled'] } } } } } }, responses: { 200: { description: 'Updated order' }, 400: { description: 'Invalid status' }, 404: { description: 'Not found' } } },
    },
