const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/property/email-branding': {
      get: { tags: ['Property'], summary: 'Get this property\'s default email branding', responses: { 200: { description: '{ logo_url, brand_color, header_bg, cancel_url } -- each null when unset. Applied to spa booking confirmation/cancellation emails whenever the request supplies no branding of its own.' } } },
      put: { tags: ['Property'], summary: 'Update default email branding (admin only)', description: 'Per field: omit = unchanged, null or empty string = clear. logo_url must be a public http(s) raster image; brand_color and header_bg are 6-digit hex colours; cancel_url may contain {id}, replaced with the appointment id at send time.', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { logo_url: { type: 'string', nullable: true }, brand_color: { type: 'string', nullable: true, example: '#c9a84c' }, header_bg: { type: 'string', nullable: true, example: '#f5f0e8' }, cancel_url: { type: 'string', nullable: true, example: 'https://example.com/cancel/?b={id}' } } } } } }, responses: { 200: { description: 'Updated branding, same shape as GET' }, 400: { description: 'Invalid URL or colour' }, 403: { description: 'Admin role required' } } },
    },
