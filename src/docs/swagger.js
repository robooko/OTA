const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/equipment/hires': {
      ...
      post: { tags: ['Equipment'], summary: 'Hire equipment', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['equipment_id', 'contact_name', 'hire_date', 'quantity'], properties: { equipment_id: { type: 'string', format: 'uuid' }, guest_id: { type: 'string', format: 'uuid' }, contact_name: { type: 'string' }, contact_email: { type: 'string' }, contact_phone: { type: 'string' }, hire_date: { type: 'string', format: 'date' }, quantity: { type: 'integer' }, notes: { type: 'string' }, golf_booking_id: { type: 'string', format: 'uuid', description: 'Link to a golf booking' }, total_price: { type: 'number', readOnly: true, description: 'Calculated at hire time (price_per_day × quantity)' } } } } } }, responses: { 201: { description: 'Hire created with total_price' }, 409: { description: 'Not enough available' } } },
    },
