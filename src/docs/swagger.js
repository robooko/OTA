const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/restaurant-orders/menu/{id}': {
      put: { tags: ['Restaurant Orders'], summary: 'Update menu item', parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { name: { type: 'string' }, description: { type: 'string' }, category: { type: 'string' }, price: { type: 'number' }, status: { type: 'string', enum: ['active', 'inactive'] }, restaurant_id: { type: 'string', format: 'uuid' }, allergens: { type: 'array', items: { type: 'string' } } } } } } }, responses: { 200: { description: 'Updated' }, 404: { description: 'Not found' } } },
    },
