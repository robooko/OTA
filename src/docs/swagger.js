const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/restaurant-web-orders': {
      get: { tags: ['Restaurant Web Orders'], summary: "\"Nando's style\" food orders: list", description: 'Ordered and paid on the website, no running tab -- distinct from /api/restaurant-orders (table/booking-required, billed through a table session).', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'restaurant_id', in: 'query', schema: { type: 'string', format: 'uuid' } }, { name: 'status', in: 'query', schema: { type: 'string', enum: ['pending', 'paid', 'preparing', 'ready', 'completed', 'cancelled'] } }, { name: 'cursor', in: 'query', schema: { type: 'string', format: 'date-time' }, description: "Oldest loaded order's created_at" }, { name: 'limit', in: 'query', schema: { type: 'integer', default: 30, maximum: 100 } }], responses: { 200: { description: 'Array of orders' } } },
      post: { tags: ['Restaurant Web Orders'], summary: 'Create a website food order', description: 'table_id is optional: omit for pickup, set for an at-table order paid per-order instead of run up on a tab. Item prices are always looked up from the live menu server-side; a variant must be one of the item\'s own variants array. No payment is taken by this call.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['restaurant_id', 'items'], properties: { restaurant_id: { type: 'string', format: 'uuid' }, table_id: { type: 'string', format: 'uuid' }, contact_name: { type: 'string' }, contact_email: { type: 'string' }, contact_phone: { type: 'string' }, scheduled_for: { type: 'string', format: 'date-time' }, notes: { type: 'string' }, items: { type: 'array', items: { type: 'object', required: ['item_id', 'quantity'], properties: { item_id: { type: 'string', format: 'uuid' }, quantity: { type: 'integer', minimum: 1 }, variant: { type: 'string' } } } } } } } } }, responses: { 201: { description: 'Created order with its line items' }, 400: { description: 'An item is missing/inactive/off this menu, an invalid variant, or a bad quantity' }, 404: { description: 'Restaurant or table not found' } } },
    },
    '/api/restaurant-web-orders/{id}': {
      get: { tags: ['Restaurant Web Orders'], summary: 'Get an order with its line items', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], responses: { 200: { description: 'Order + items' }, 404: { description: 'Not found' } } },
      put: { tags: ['Restaurant Web Orders'], summary: "Update an order's kitchen status and/or contact details", description: "Per field: omit = unchanged. Covers kitchen workflow (paid -> preparing -> ready -> completed, or cancelled) and a checkout flow patching in real contact details before confirm-payment -- createOrder accepts no contact_name so a PaymentIntent can exist before that form even renders.", security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { status: { type: 'string', enum: ['pending', 'paid', 'preparing', 'ready', 'completed', 'cancelled'] }, contact_name: { type: 'string' }, contact_email: { type: 'string' }, contact_phone: { type: 'string' }, notes: { type: 'string' } } } } } }, responses: { 200: { description: 'Updated order' }, 400: { description: 'Invalid status' }, 404: { description: 'Not found' } } },
    },
    '/api/restaurant-web-orders/{id}/payment-intent': {
      post: { tags: ['Restaurant Web Orders'], summary: 'Create (or reuse) a Stripe PaymentIntent for the order total', description: "Same pattern as restaurant table sessions' 'online' channel and proshop orders: a regular (not card_present) PaymentIntent on the property's own Stripe account, embeddable via Stripe Elements. Idempotent -- a retry reuses the existing intent unless it's already succeeded/cancelled.", security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], responses: { 200: { description: '{ client_secret, payment_intent_id, amount } in minor units, or { already_paid: true, payment_intent_id } if a prior charge already succeeded' }, 404: { description: 'Order not found' }, 409: { description: 'No Stripe key configured, order cancelled, or already paid' }, 502: { description: 'Stripe error' } } },
    },
    '/api/restaurant-web-orders/{id}/confirm-payment': {
      post: { tags: ['Restaurant Web Orders'], summary: 'Verify a succeeded PaymentIntent and mark the order paid', description: 'Re-checks the PaymentIntent directly with Stripe (status and amount) before trusting a client-side "it succeeded" claim.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['payment_intent_id'], properties: { payment_intent_id: { type: 'string' } } } } } }, responses: { 200: { description: 'Order, now status/payment_status = paid' }, 404: { description: 'Not found' }, 409: { description: "Intent doesn't match this order, hasn't succeeded, or amount mismatch" }, 502: { description: 'Stripe error' } } },
    },
    // ── Pro Shop ──────────────────────────────────────────────────────────────
    '/api/proshop/shops': {
