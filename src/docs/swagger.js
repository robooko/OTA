const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/property/google-reviews': {
      get: { tags: ['Property'], summary: "Live Google reviews for this property's website", description: 'Rating, count, latest quotes, maps link and write-a-review link for the property\'s google_place_id, fetched from the Places API and cached server-side (12h TTL; stale cache served on Google failure). 404 when no place id is set; 503 when the feature is turned off (GOOGLE_REVIEWS_ENABLED is not "true" -- the default) or the server has no GOOGLE_PLACES_API_KEY and nothing is cached.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], responses: { 200: { description: '{ rating, count, maps_url, write_review_url, reviews: [{ author, author_photo, rating, text, relative_time, published_at }], fetched_at }' }, 404: { description: 'No Google place id configured for this property' }, 502: { description: 'Google fetch failed and no cache exists' }, 503: { description: 'Feature not configured on this server and no cache exists' } } },
    },
    '/api/property/google-place-id': {
      put: { tags: ['Property'], summary: "Set this property's Google Business Profile place id (API-key rail)", description: 'Used by onboarding tooling; null clears it. Changing the place id drops the reviews cache.', security: [{ bearerAuth: [] }, { apiKeyAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['place_id'], properties: { place_id: { type: 'string', nullable: true, maxLength: 300 } } } } } }, responses: { 200: { description: '{ id, google_place_id }' }, 400: { description: 'place_id missing or invalid' } } },
    },
