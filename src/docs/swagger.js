const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/billing/webhooks/stripe': {
      post: { tags: ['Billing'], summary: 'Stripe webhook (platform account) -- credits token purchases', description: 'Verified with PLATFORM_STRIPE_WEBHOOK_SECRET against the raw body. Handles checkout.session.completed / checkout.session.async_payment_succeeded; everything else is acknowledged and ignored. Idempotent per session.', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object' } } } }, responses: { 200: { description: '{ received: true }' }, 400: { description: 'Bad signature' }, 500: { description: 'Credit failed -- Stripe will retry' } } },
    },
